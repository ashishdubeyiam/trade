...
5.  **Access the UI:** Open your web browser and go to the address shown in the Flask development server terminal, usually:
    ...
    ```
6.  **Using the UI:**
    - The main page allows you to upload a CSV transaction data file.
    - Upon uploading, the backend triggers the ML pipeline located in `fraud_detection_project/src/main.py` using the uploaded file.
    - After processing, results (metrics) will be displayed on a new page.
    - Uploaded files are temporarily stored in `fraud_detection_ui/uploads/` and are cleaned up after processing.
    - ML models and results (JSON, plots) generated by the pipeline will be saved within the `fraud_detection_project/models/` and `fraud_detection_project/results/` directories, respectively.
